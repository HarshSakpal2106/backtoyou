// ─── STATIC DEMO PETS (fallback / always shown) ───────────────────────────────
// These are your original hardcoded pets — they always show up alongside real DB reports
...
async function fetchPetsFromDB(category) {
  try {
    const res = await fetch(`${API_BASE}/api/${category}-pets`);
    if (!res.ok) return [];
    return await res.json();
  } catch (err) {
    console.error("Could not fetch pets from DB:", err);
    return []; // silently fall back to demo pets only
  }
}

async function getAllPetsByCategory(category) {
  const demoPets = category === "lost" ? lostPets : foundPets;
  const dbPets = await fetchPetsFromDB(category);
  // DB pets first (newest), then demo pets
  return [...dbPets, ...demoPets];
}
...
async function applyLostPetFilters() {
  ...
  const allPets = await getAllPetsByCategory(category);
...
async function removePetReport(dbId, category) {
  if (!confirm("Mark this pet as reunited? This will remove the listing.")) return;

  try {
    const res = await fetch(`${API_BASE}/api/${category}-pets/${dbId}`, {
      method: "DELETE",
...
function initReportPage() {
  ...
  function handleFormSubmit(form, category) {
    ...
    form.addEventListener("submit", async function (e) {
      ...
      // FormData already includes the file — send directly to backend
      ...
      try {
        const res = await fetch(`${API_BASE}/api/${category}-pets`, {
          method: "POST",
          headers: { Authorization: "Bearer " + getToken() },
          // ⚠️ Do NOT set Content-Type — browser sets it with the boundary for multipart
          body: formData,
        });
        ...
        if (!res.ok) {
          ...
          submitBtn.textContent = `Submit ${capitalizeFirst(category)} Pet Report`;
          return;
        }
...
function showPetModal(pet) {
  ...
  if (sendBtn) {
    sendBtn.addEventListener("click", () => {
      ...
      emailjs
        .send("service_back2you", "template_contact_owner", {
          to_email: toEmail,
